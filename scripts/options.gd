...
func _ready() -> void:
	...
	# Animations & Background Initialization
...
func _setup_button_animations() -> void:
	# We put it in an array to easily add more buttons later if needed
...
func _process(delta: float) -> void:
	# If the menu is hidden, we don't need to waste resources calculating parallax
	if not visible:
		return
