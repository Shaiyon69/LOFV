...
# NEW: Called by the Boss when it touches the tree
func destroy() -> void:
	if is_destroyed:
		return
	...
	# Turn off the physics so it no longer blocks the player or enemies
	collision_shape.set_deferred("disabled", true)
	transparency_zone.monitoring = false
	
	# Optional: Add a crunching sound effect here!
	# AudioManager.play_sfx_2d(...) 
	
	# Make it look dead/crushed (fade it out and make it dark)
	var tween = create_tween()
