...
var active_radius: float = 1000.0 # NEW: Distance where slimes become solid and smart
	...
	if player:
		...
		if is_hurt:
			...
			if distance > active_radius:
				# --- OFF-SCREEN MODE (GHOSTS) ---
				# Turn OFF collision with Layer 2 (Trees) so they phase through
				set_collision_mask_value(2, false)
				
				# Just move in a straight line, ignore whiskers and soft collision
				velocity = direction * speed
			else:
				# --- ON-SCREEN MODE (SOLID) ---
				# Turn ON collision with Layer 2 (Trees)
...
func _update_expensive_logic() -> void:
	...
	# ONLY calculate soft-collision if they are on-screen (Massive CPU saver!)
	if distance <= active_radius:
